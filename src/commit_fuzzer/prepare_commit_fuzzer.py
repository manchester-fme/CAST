...
class PrepareCommitAnalyzer:
    def __init__(self, repo_path: str = ".", compile_commands: Optional[str] = None,
                 namespace_prefix: str = "", clang_args: Optional[List[str]] = None):
        """Initialize with repository path.

        namespace_prefix and clang_args are the only two solver-specific
        inputs this analyzer needs (e.g. "cvc5::" and cvc5's own include
        paths/preprocessor defines) - see manifest.json's fuzzer.namespace_prefix
        / fuzzer.clang_args. clang_args is only used as a fallback when
        compile_commands doesn't have an entry for a given file.
        """
    ...
    def get_qualified_name(self, cursor) -> str:
        ...
        # Ensure we have the full namespace prefix (e.g. cvc5::) at the start,
        # trimming any outer wrapping namespace/qualifiers before it
        if self.namespace_prefix and not qualified_name.startswith(self.namespace_prefix):
            if self.namespace_prefix in qualified_name:
                prefix_index = qualified_name.find(self.namespace_prefix)
                qualified_name = qualified_name[prefix_index:]
        
        return qualified_name
    
    def is_target_function(self, signature: str) -> bool:
        """Check if a function signature belongs to this solver.
        ...
        try:
            ...
            if head.startswith('std::') or head.startswith('__') or head.startswith('__gnu_cxx::'):
                return False
            # Include any functions within this solver's own namespace
    ...
    def get_commit_functions(self, commit_hash: str) -> tuple[List[str], List[str]]:
        ...
        for file_path, changed_lines in changed_files_lines.items():
            ...
            if after_funcs:
                target_funcs = [f for f in after_funcs if self.is_target_function(f.signature)]
                for ln in sorted(changed_lines):
                    candidates = [f for f in target_funcs if int(f.start) <= ln <= int(f.end)]
                    if not candidates:
                        continue
        ...
        try:
            ...
            def visit(n):
                if n.kind in [clang.cindex.CursorKind.FUNCTION_DECL, clang.cindex.CursorKind.CXX_METHOD] and n.is_definition():
                    ...
                    if sig and node_file and self.is_target_function(sig):
    ...
    def _build_clang_args(self) -> List[str]:
        """Build clang arguments for this solver's parsing (manifest-declared
        fuzzer.clang_args - see __init__). Fallback path only, used when a
        file has no entry in compile_commands.json."""
...
def main():
    ...
    parser.add_argument('--namespace-prefix', default='',
                       help="This solver's own C++ namespace prefix (e.g. \"cvc5::\") - "
                            "from manifest.json's fuzzer.namespace_prefix")
