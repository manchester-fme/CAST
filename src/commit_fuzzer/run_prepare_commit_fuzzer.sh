...
# Script to run commit coverage analysis for a solver's commits (any solver
# whose manifest.json declares a namespace_prefix/clang_args - see
# prepare_commit_fuzzer.py in this same directory).
...
Run commit coverage analysis for a solver's commits.

ARGUMENTS:
    COMMITS_TO_ANALYZE    Number of commits to analyze (default: 3)

OPTIONS:
    -p, --python-script PATH     Path to prepare_commit_fuzzer.py (default: alongside this script)
    ...
    --commit-hash HASH           Commit to analyze (default: current HEAD)
    --namespace-prefix PREFIX    Solver's own C++ namespace prefix, e.g. "cvc5::"
                                  (from manifest.json's fuzzer.namespace_prefix)
    --clang-args-json JSON       JSON array of fallback clang args
                                  (from manifest.json's fuzzer.clang_args)
...
# Use HEAD commit if --commit-hash is not set, otherwise use the specified commit
...
for commit in "${COMMITS[@]}"; do
    ...
    if [ -n "$NAMESPACE_PREFIX" ]; then
        PYTHON_ARGS+=("--namespace-prefix" "$NAMESPACE_PREFIX")
    fi

    if [ -n "$CLANG_ARGS_JSON" ]; then
        PYTHON_ARGS+=("--clang-args-json" "$CLANG_ARGS_JSON")
    fi
