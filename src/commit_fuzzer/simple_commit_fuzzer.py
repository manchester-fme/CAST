...
class SimpleCommitFuzzer:
    ...
    def _resolve_binary(value: str) -> str:
        """Resolve a solver binary reference that's either a filesystem path
        (from a build this workflow fetched/compiled) or a bare command name
        to find on PATH (from a pip-installed/stable-release solver, e.g.
        the z3-solver package for a default, non-oracle_solver_dir oracle).
        Raises ValueError if neither resolves."""
        if Path(value).exists():
            return value
        if shutil.which(value.split()[0]):
            return value
    ...
    def _validate_solvers(self):
        ...
        self._resolve_binary(self.target_path)
        if self.oracle_path:
            self._resolve_binary(self.oracle_path)
    ...
    def run(self):
        ...
        print(f"Solvers: {self._get_solver_clis()}")
        print()
