...
Examples:
  ...
  python3 src/coverage/coverage_state.py cvc5 check --test-count 12345 --output decision.json

  # Update state after successful build
  python3 src/coverage/coverage_state.py cvc5 update --test-count 12345 --commit-hash abc123

  # Get current state (debugging)
  python3 src/coverage/coverage_state.py cvc5 get

Environment variables:
  STORAGE_PROVIDER: "r2" (default) or "aws"
  BUCKET_NAME: bucket name, whichever backend is active
  AWS_REGION / AWS_ENDPOINT_URL_S3: set by src/configure_storage.sh
        """
    )
    ...
    # when STORAGE_PROVIDER=r2 — see src/storage_backend.py)
    try:
        bucket = get_bucket()
