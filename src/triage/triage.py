...
def stage4_report(final_triggers, target_cmd, oracle_cmd, post, repo, commit_hash):
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--commit-hash",
        help="solver commit these triggers were found against, for the filed issue's "
        "'commit:' line (see report.py --commit-hash)",
    )
